...
class Ui_MainWindow(object):
    ...
    def load_signal_file(self):
        ...
        self.signal_df.insert(1, 'anno_key', 0)
    ...
    def create_key(self):
        anns,ok = QInputDialog.getInt(MainWindow,"Number of Classifications","Enter Number of Classifications", min =1, max=15)
        if ok:
            self.class_num = anns
    ...
    def load_project(self):
        ...
        print(self.loaded_signal_file)
        self.signal_df = pd.read_csv(self.loaded_signal_file)
        self.signalWidget.plot(x= self.signal_df.index.values, y=self.signal_df.iloc[:,3], pen = "b")
        self.signalWidget.plot(x= self.signal_df.index.values, y=self.signal_df.iloc[:,4], pen = "r")
        self.signalWidget.plot(x= self.signal_df.index.values, y=self.signal_df.iloc[:,5], pen = 'g')
